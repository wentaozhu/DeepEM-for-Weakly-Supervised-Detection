...
class DPN(nn.Module):
    def __init__(self, cfg):
        ...
        self.last_planes = 736
        self.layer5 = self._make_layer(in_planes[2], out_planes[2], num_blocks[2], dense_depth[2], stride=1)
        self.last_planes = 656+3
        self.layer6 = self._make_layer(in_planes[1], out_planes[1], num_blocks[1], dense_depth[1], stride=1)
        self.linear = nn.Linear(out_planes[3]+(num_blocks[3]+1)*dense_depth[3], 2)#10)
        self.last_planes = 256
        self.path1 = nn.Sequential(
            nn.ConvTranspose3d(self.last_planes, self.last_planes, kernel_size = 2, stride = 2),
            nn.BatchNorm3d(self.last_planes),
            nn.ReLU(inplace = True))
        self.last_planes = 480
        self.path2 = nn.Sequential(
            nn.ConvTranspose3d(self.last_planes, self.last_planes, kernel_size = 2, stride = 2),
            nn.BatchNorm3d(self.last_planes),
            nn.ReLU(inplace = True))
        self.drop = nn.Dropout3d(p = 0.5, inplace = False)
        self.output = nn.Sequential(nn.Conv3d(176, 64, kernel_size = 1),
                                    nn.ReLU(),
                                    #nn.Dropout3d(p = 0.3),
...
def DPN92_3D():
    cfg = {
        'in_planes': (64,64,96,128),#(96,192,384,768),
        'out_planes': (96,96,128,128),#(256,512,1024,2048),
        'num_blocks': (3,4,10,3),
        'dense_depth': (16,16,32,32)
    }
    return DPN(cfg)
