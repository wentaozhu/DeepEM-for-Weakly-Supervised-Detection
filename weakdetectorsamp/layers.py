...
class Fprclsloss(nn.Module):
    def __init__(self, num_hard=2):
        super(Fprclsloss, self).__init__()
    ...
    def forward(self, output, labels, train=True):
        ...
        pos_output = output[pos_idcs].view(-1, 1)
        pos_labels = labels[pos_idcs].view(-1, 1)
...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train = True, prob=None, isp=False):
        batch_size = labels.size(0)
        ...
        if isp:
            # print(prob.numpy()[0,0])
            return [loss*(prob.numpy()[0,0]), classify_loss_data] + regress_losses_data + [pos_correct, pos_total, neg_correct, neg_total]
...
class GetPBB(object):
    ...
    def __call__(self, output,thresh = -3, ismask=False, weaklst=None, model2=None):
        stride = self.stride
        ...
        output[:, :, :, :, 1] = oz.reshape((-1, 1, 1, 1)) + output[:, :, :, :, 1] * anchors.reshape((1, 1, 1, -1)) # z
        ...
        if weaklst and model2:
            # mask = (output[..., 1] >= 0)
            ret = []
            for wl in weaklst:
                # print(type(output[..., 1]))
                # print(output[..., 1].shape)
                mask = (output[..., 1] >= wl[2]) & (output[..., 1] <= wl[3])
                if mask.shape[0] == 0 or len(np.where(mask))==1:
                    ret.append(np.array([]))
                    continue
                # print(mask.shape, len(np.where(mask)))
                xx, yy, zz, aa = np.where(mask)
                output = output[xx, yy, zz, aa] # prob, z, x, y, d
                feat = np.hstack([output[:,1].reshape((-1,1)) / wl[4], output[:,2].reshape((-1,1)) / wl[5], output[:,3].reshape((-1,1)) / wl[6]])
                if feat.shape[0] == 0:
                    ret.append(np.array([]))
                    continue
                # print(output.shape, type(output), feat.shape, wl[0], xx.shape, type(xx))
                pred = model2.forward(Variable(torch.from_numpy(feat).float(), requires_grad=False)).data.numpy()
                # print(pred.shape, type(pred.argmax(axis=1)==wl[0]), np.sum(pred.argmax(axis=1)==wl[0]), np.min(pred.argmax(axis=1)), np.max(pred.argmax(axis=1)))
                xxlst = np.where(pred.argmax(axis=1)==wl[0])
                if not xxlst or xxlst[0].shape[0]==0: 
                    ret.append(np.array([]))
                    continue
                # print(len(xxlst), xxlst[0].shape, type(xxlst[0]), xxlst[0][0])
                output = np.array(output[(pred.argmax(axis=1)==wl[0])!=0, :])
                # print(output.shape)
                output = np.array(output[output[:,0] > thresh, :])
                # print(output.shape)
                pbb = nms(output, 0.1) # prob, z, x, y, d
                if pbb.shape[0] == 0: 
                    ret.append(pbb)
                    continue
                # print(pbb.shape)
                feat = np.hstack([pbb[:,1].reshape((-1,1)) / wl[4], pbb[:,2].reshape((-1,1)) / wl[5], pbb[:,3].reshape((-1,1)) / wl[6]])
                # print(feat.shape)
                linearout = model2.forward(Variable(torch.from_numpy(feat).float(), requires_grad=False))
                # print(linearout.size())
                linearprob = self.softmax.forward(linearout)
                # print(linearprob.size())
                prob = 1/(1+np.exp(-pbb[:,0])) * 2*np.exp(-np.square(np.maximum(abs(wl[1]-pbb[:,1])-3.253443196/2, np.zeros_like(pbb[:,1])))/(2*(4.76/2)*(4.76/2)))/(np.sqrt(2*np.pi*(4.76/2)*(4.76/2))) * \
                    linearprob.data.numpy()[:,wl[0]]
                pbb = np.array(pbb[(-prob).argsort()])
                prob = np.array(prob[(-prob).argsort()])
                sum_prob = np.sum(prob)
                cur_prob = 0.0
                currand = np.random.rand()
                for idx in range(pbb.shape[0]):
                    cur_prob += prob[idx]
                    if cur_prob / sum_prob > currand:
                        ret.append(pbb[idx, :])
            return ret
