...
def main():
    ...
    if args.resume:
        ...
        if args.test!=1 and start_epoch!=0:
            print('resume mode from ', args.resume[:-len('weak014.ckpt')]+args.resume[-len('014.ckpt'):])
            checkpoint = torch.load(args.resume[:-len('weak014.ckpt')]+args.resume[-len('014.ckpt'):])
            net.load_state_dict(checkpoint['state_dict'])
    # else:
    if start_epoch == 0:
        start_epoch = 1
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for i, (data, target, coord, nzhw) in enumerate(weaktest_loader): # check data consistency
            if i >= len(testfilelist)/args.batch_size:
                break
        srslst, cdxlst, cdylst, cdzlst, dimlst, prblst, poslst = weaktest(weaktest_loader, model2, net, get_pbb, save_dir, config, epoch)
        config['ep'] = epoch
        ...
        dataset = weakdatav2.DataBowl3Detector(
            ...
            list(set(fnmlst)),
            config,
            phase = 'train', fnmlst=srslst, cdxlst=cdxlst, cdylst=cdylst, cdzlst=cdzlst, dimlst=dimlst, prblst=prblst, poslst=poslst)
...
def weaktest(data_loader, model2, net, get_pbb, save_dir, config, epoch):
    ...
    csvwriter.writerow(['seriesuid', 'coordX', 'coordY', 'coordZ', 'diameter', 'probability', 'position'])
    ...
    for i_name, (data, target, coord, nzhw) in enumerate(data_loader):
        ...
        for idx, wl in enumerate(weakdct[name]):
            if len(pbb[idx]) == 0: continue #or pbb[idx].shape[0]==0
            # p = 1/(1+np.exp(-pbb[0,0])) * prob[0,0]
            annocount += 1
            if len(pbb[idx]) == 2:
                srslst.append(name)
                cdxlst.append(pbb[idx][0][2])
                cdylst.append(pbb[idx][0][3])
                cdzlst.append(pbb[idx][0][1])
                dimlst.append(pbb[idx][0][4])
                prblst.append(1.0)#pbb[idx][0][0])
                poslst.append(wl[0])
                srslst.append(name)
                cdxlst.append(pbb[idx][1][2])
                cdylst.append(pbb[idx][1][3])
                cdzlst.append(pbb[idx][1][1])
                dimlst.append(pbb[idx][1][4])
                prblst.append(pbb[idx][1][0])
                poslst.append(wl[0])
                # print(cdxlst[-1], cdylst[-1], cdzlst[-1], dimlst[-1], prblst[-1], poslst[-1])
            else:
                srslst.append(name)
                ...
                prblst.append(1.0)#pbb[idx][0])
                poslst.append(wl[0])
            # csvwriter.writerow([name, pbb[idx][2], pbb[idx][3], pbb[idx][1], pbb[idx][4], pbb[idx][0], wl[0]]) # x, y, z, d, p, pos
        # np.save(os.path.join(save_dir, name+'_pbb.npy'), pbb)
        # np.save(os.path.join(save_dir, name+'_lbb.npy'), lbb)
    ...
    print
    print
    return srslst, cdxlst, cdylst, cdzlst, dimlst, prblst, poslst
