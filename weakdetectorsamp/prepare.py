...
def full_prep(train=True, val=True, test=True):
    ...
    if not os.path.exists(finished_flag):
        ...
        if train:
            ...
            # for idx in xrange(len(trainfilelist)):
            # 	savenpy(idx, trainalllabel, trainalllabel, train_data_path, train_prep_folder)
            pool = Pool(30)
            partial_savenpy = partial(savenpy, annos=trainalllabel, filelist=trainalllabel, data_path=train_data_path, prep_folder=train_prep_folder)
            N = len(trainfilelist)
            # savenpy(1)
...
#     resolution = np.array([2,2,2])
    ...
    if isClean:
        ...
        sliceim2 = sliceim1[extendbox[0,0]:extendbox[0,1],
                    ...
                    extendbox[2,0]:extendbox[2,1]]
        sliceim = sliceim2[np.newaxis,...]
        np.save(os.path.join(savepath, fnamedct[name]+'_clean.npy'), sliceim)
        np.save(os.path.join(savepath, fnamedct[name]+'_spacing.npy'), spacing)
        np.save(os.path.join(savepath, fnamedct[name]+'_extendbox.npy'), extendbox)
        np.save(os.path.join(savepath, fnamedct[name]+'_origin.npy'), origin)
        np.save(os.path.join(savepath, fnamedct[name]+'_mask.npy'), Mask)
    ...
    if islabel:
        ...
        if len(label)==0:
            ...
            label2[:3] = label2[:3]-np.expand_dims(extendbox[:,0],1)
            label2 = label2[:4].T
        np.save(os.path.join(savepath,fnamedct[name]+'_label.npy'), label2)
        
    print(name, fnamedct[name])
...
if __name__=='__main__':
    full_prep(train=True, val=True, test=False)
    # preprocess_luna()