...
debug = False #True#False#True#True #True#False #True
...
class Bottleneck(nn.Module):
    def __init__(self, last_planes, in_planes, out_planes, dense_depth, stride, first_layer):
        ...
        self.conv2 = nn.Conv3d(in_planes, in_planes, kernel_size=3, stride=stride, padding=1, groups=1, bias=False)
        self.bn2 = nn.BatchNorm3d(in_planes)
        self.conv3 = nn.Conv3d(in_planes, out_planes+dense_depth, kernel_size=1, bias=False) # 1
...
class DPN(nn.Module):
    def __init__(self, cfg):
        ...
        self.last_planes = 136
        self.layer5 = self._make_layer(64, 64, num_blocks[2], dense_depth[2], stride=1)
        self.last_planes = 128+3
        self.layer6 = self._make_layer(128, 128, num_blocks[1], dense_depth[1], stride=1)
        ...
        self.output = nn.Sequential(nn.Conv3d(138, 64, kernel_size = 1),
                                    nn.ReLU(),
                                    #nn.Dropout3d(p = 0.3),
...
def DPN92_3D():
    cfg = {
        'in_planes': (24,32,42,64),#(96,192,384,768),
        'out_planes': (24,32,42,64),#(256,512,1024,2048),
        'num_blocks': (3,4,10,3),
        'dense_depth': (2,2,2,2)
    }
    return DPN(cfg)
