    ...
    def __call__(self, output,thresh = -3, ismask=False, weaklst=None, model2=None):
        ...
        if weaklst and model2:
            ...
            for wl in weaklst:
                ...
                prob = 1/(1+np.exp(-pbb[:,0])) * 2*np.exp(-np.square(np.maximum(abs(wl[1]-pbb[:,1])-3.253443196/2, 0))/(2*(4.76/2)*(4.76/2)))/(np.sqrt(2*np.pi*(4.76/2)*(4.76/2))) * \
                    linearprob.data.numpy()[:,wl[0]]
                argmaxidx = np.argmax(prob)
                pbb[argmaxidx, 0] = np.max(prob)
                ret.append(pbb[np.argmax(prob), :])
            return ret
