...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, labelfname, config, phase='train', split_comber=None):
        ...
        pdfrm = pd.read_csv(labelfname, names=['fname', 'position', 'centerslice'])
        calibfnamelst = pdfrm['fname'].tolist()[1:]
        calibpositionlst = pdfrm['position'].tolist()[1:]
        calibcenterslicelst = pdfrm['centerslice'].tolist()[1:]
        calibdict = {}
        for idx, calibfname in enumerate(calibfnamelst):
            if calibfname not in calibdict:
                calibdict[calibfname] = [[calibpositionlst[idx], calibcenterslicelst[idx]]]
            else:
                calibdict[calibfname].append([calibpositionlst[idx], calibcenterslicelst[idx]])
        labels = []
        print len(idcs)
        for idx in idcs:
            labels.append(calibdict[idx])
        ...
        if self.phase != 'test':
            ...
            for i, l in enumerate(labels):
                ...
                if len(l) > 0 :
                    for t in l:
                        if len(t) != 2: print(t,i)
                        self.bboxes.append([np.concatenate([[i],np.array([t[0], t[1]])])])
