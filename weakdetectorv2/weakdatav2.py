...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None,fnmlst=None, cdxlst=None, cdylst=None, \
        cdzlst=None, dimlst=None, prblst=None, poslst=None, lwzlst=None, upzlst=None):
        ...
        if phase!='test':
            ...
            annodct = {}
            if not fnmlst:
                pdfrm = pd.read_csv(config['save_dir']+'weakinferep'+str(config['ep'])+'.csv', names=['seriesuid', 'coordX', 'coordY', 'coordZ', 'diameter', 'probability', 'position', 'lwz', 'upz'])
                ...
                lwzlst = pdfrm['lwz'].tolist()[1:]
                upzlst = pdfrm['upz'].tolist()[1:]
            for idx, fnm in enumerate(fnmlst):
            	value = [float(cdzlst[idx]),float(cdxlst[idx]),float(cdylst[idx]),float(dimlst[idx]),float(prblst[idx]),int(poslst[idx]), float(lwzlst[idx]), float(upzlst[idx])]
        ...
        for idx in idcs:
            ...
            labels2.append(np.array(l[:,-4:]))
            labels.append(np.array(l[:,:-4]))
        ...
        if self.phase == 'train':
            ...
            print('#val', len(self.bboxes)/samplerate)#10)
        else:
            print('#test', len(self.sample_bboxes)/samplerate)#10)
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                sample, target, bboxes, coord, lwz, upz = self.crop(imgs, bbox[1:], bboxes, isScale,isRandom, prob[0,-2], prob[0,-1])
                if self.phase=='train' and not isRandom:
                     sample, target, bboxes, coord, lwz, upz = augment(sample, target, bboxes, coord,
                        ifflip = self.augtype['flip'], ifrotate=self.augtype['rotate'], ifswap = self.augtype['swap'], lwz=lwz, upz=upz)
            else:
                ...
                isScale = self.augtype['scale'] and (self.phase=='train')
                sample, target, bboxes, coord, lwz, upz = self.crop(imgs, [], bboxes,isScale=False,isRand=True, lwz=prob[0,-2], upz=prob[0,-1])
            ...
            label = self.label_mapping(sample.shape[1:], target, bboxes, filename, lwz, upz)
            # print(label.shape)
            ...
            if torch.from_numpy(np.array([prob[0,0]])).size()[0] == 0:
                ...
                assert 1 == 0
            # print feat.shape, type(feat), sample.shape, label.shape, coord.shape
            if not isprb:
...
def augment(sample, target, bboxes, coord, ifflip = True, ifrotate=True, ifswap = True, lwz=None, upz=None):
...
#         flipid = np.array([np.random.randint(2),np.random.randint(2),np.random.randint(2)])*2-1
        ...
        for ax in range(3):
            if flipid[ax]==-1:
                ...
                if ax == 0 and lwz is not None:
                    lwz = sample.shape[ax+1] - upz
                    upz = sample.shape[ax+1] - lwz
    return sample, target, bboxes, coord, lwz, upz
...
class Crop(object):
    def __init__(self, config):
        ...
        self.stride = config['stride']
        self.pad_value = config['pad_value']
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False, lwz=None, upz=None):
        if isScale:
            radiusLim = [8.,120.]
            scaleLim = [0.75,1.25]
        ...
        if lwz is not None:
            ...
            lwz = lwz * scale
            upz = upz * scale
        return crop, target, bboxes, coord, lwz, upz
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes, filename, lwz=None, upz=None):
        lwz, upz = (lwz+upz)/2. - 5., (lwz+upz)/2. + 5.
        stride = self.stride
        ...
        lwzlst = [lwz]*len(anchors)
        upzlst = [upz]*len(anchors)
        for bbox in bboxes:
            for i, anchor in enumerate(anchors):
                # print select_samples(bbox, anchor, th_neg, oz, oh, ow, lwz=lwz, upz=upz)
                if lwz is not None:
                    iz, ih, iw, lwzlst[i], upzlst[i] = select_samples(bbox, anchor, th_neg, oz, oh, ow, lwz=lwz, upz=upz)
                else:
        ...
        if self.phase == 'train' and self.num_neg > 0:
            ...
            orgneg_z = np.array(neg_z)
            if lwz is not None:
                for i, anchor in enumerate(anchors):
                    # assert lwzlst[i] <= upzlst[i]
                    if i == 0:
                        mz1 = np.logical_and(neg_a == i, orgneg_z <= lwzlst[i])
                        mz2 = np.logical_and(neg_a == i, orgneg_z >= upzlst[i])
                        mz = np.logical_or(mz1, mz2)
                        iz = np.where(mz)[0]
                        neg_z = np.array(orgneg_z[iz])
                    else:
                        mz1 = np.logical_and(neg_a == i, orgneg_z <= lwzlst[i])
                        mz2 = np.logical_and(neg_a == i, orgneg_z >= upzlst[i])
                        mz = np.logical_or(mz1, mz2)
                        iz = np.where(mz)[0]
                        neg_z = np.concatenate((orgneg_z[iz], neg_z))
                # print(orgneg_z.shape, neg_z.shape, lwzlst, upzlst, orgneg_z.max(), orgneg_z.min(), neg_a[0], anchors)
...
def select_samples(bbox, anchor, th, oz, oh, ow, lwz=None, upz=None):
    z, h, w, d = bbox
    max_overlap = min(d, anchor)
    min_overlap = np.power(max(d, anchor), 3) * th / max_overlap / max_overlap
    if min_overlap > max_overlap:
        if lwz is not None: return np.zeros((0,), np.int64), np.zeros((0,), np.int64), np.zeros((0,), np.int64), 0, 0
        return np.zeros((0,), np.int64), np.zeros((0,), np.int64), np.zeros((0,), np.int64)
    else:
        if lwz is not None:
            s = lwz - 0.5 * np.abs(d - anchor) - (max_overlap - min_overlap)
            e = lwz + 0.5 * np.abs(d - anchor) + (max_overlap - min_overlap)
            mz = np.logical_and(oz >= s, oz <= e)
            iz = np.where(mz)[0]
            if len(iz) == 0: lwz = oz.shape[0]
            else: lwz = iz.min()
            s = upz - 0.5 * np.abs(d - anchor) - (max_overlap - min_overlap)
            e = upz + 0.5 * np.abs(d - anchor) + (max_overlap - min_overlap)
            mz = np.logical_and(oz >= s, oz <= e)
            iz = np.where(mz)[0]
            if len(iz) == 0: upz = 0
            else: upz = iz.max()
            upz, lwz = max(upz, lwz), min(upz, lwz)
        ...
        ih = ih[mask]
        iw = iw[mask]
        if lwz is not None: return iz, ih, iw, lwz, upz
        return iz, ih, iw
